...
sub _file_instance {
    my ($self, %arg) = @_;
    if ($arg{'path'} =~ m{^(?:.*/)?metacoll\.([^.]+)\.(new|updates|deletes)\.[DWMQ](\d\d\d\d)(\d\d)(\d\d)\.T(\d\d)(\d\d)(\d\d)(?:\.(.+))?\.(\d)\.([a-z]+)$}) {
        ...
        @arg{qw(date time year month day hour minute second)} = ($Y.$m.$d.'T'.$H.$M.$S, $H.$M.$S, $Y, $m, $d, $H, $M, $S);
